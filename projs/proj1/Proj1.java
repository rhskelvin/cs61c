...
public class Proj1{
        ...
        @Override
            public void map(WritableComparable docID, Text docContents, Context context)
                ...
                for(int i = 0; matcher.find();i++){
                    ...
                    wordList.add(word);
                    if(word.equals(targetGram)) indicesOfTarget.add(i);
                }
                int wordNum = wordList.size();
                for(String i :wordList) distanceList.add(Double.POSITIVE_INFINITY); // initialize everything with max value
                if (indicesOfTarget.size()==0){
                        for(int i = 0; i < wordList.size();i++)
                            context.write(new Text(wordList.get(i)), new DoublePair(1, func.f(Double.POSITIVE_INFINITY )));
                }
                else for(int i = 0; i < wordList.size();i++){
                        double d = Math.abs(i-indicesOfTarget.get(0));
                        for(double keyIndex:indicesOfTarget)
                             if(Math.abs(i-keyIndex)<d) d = Math.abs(i-keyIndex);
                        if(!targetGram.equals(wordList.get(i))) context.write(new Text(wordList.get(i)), new DoublePair(1, func.f(d)));
                }        
    }
    ...
    public static class Combine1 extends Reducer<Text, DoublePair, Text, DoublePair> {
        ...
        @Override
          ...
          for (DoublePair p: values) {
            ...
            if(aw!=0) context.write(key, new DoublePair(aw, sw));
            // System.out.println("the word '"+key+"' appears "+t+" times");
            }
    }
    ...
    public static class Reduce1 extends Reducer<Text, DoublePair, DoubleWritable, Text > {
        ...
        public void reduce(Text key, Iterable<DoublePair> values,
                ...
                // YOUR CODE HERE
            //System.out.println("** Reducer1 is processing word: '"+key+"'");
                double co_occurrence = 0;
                double sw = 0; // sum of f(d), S value
                double aw = 0; // word count, A value
                // int t = 0;
                for (DoublePair p:values ) {
                    sw+=p.getDouble2();
                    aw+=p.getDouble1();  
                ...
                // if(t!=0)System.out.println("the word '"+key+"' appears "+t+" times");
                //System.out.println("combiner is working");
                co_occurrence = sw * Math.log(sw)*Math.log(sw)*Math.log(sw)/aw;
                if(sw!=0) context.write( new DoubleWritable(co_occurrence*(-1)), key); // the -1 makes sorting in descending order
    else {
    ...
    public static class Reduce2 extends Reducer<DoubleWritable, Text, DoubleWritable, Text> {
        ...
        @Override
            public void reduce(DoubleWritable key, Iterable<Text> values,
                 ...
                 // YOUR CODE HERE
                for (Text t:values ) {
                    //System.out.println("reduce is working on '"+t+"'");
                    if(n<N_TO_OUTPUT){
                             context.write(new DoubleWritable(Math.abs(key.get())), t);            
                    }
                     n++;
                }
            }

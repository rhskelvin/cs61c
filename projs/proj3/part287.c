...
{   
    ...
    #pragma omp parallel shared(kernel, out, in)
    {
        ...
        for (int y = 1; y < data_size_Y-1; y++){
            ...
            for(int rowNum = -1; rowNum <= 1; rowNum++){
                float *firsttemp =  (y+rowNum)*data_size_X+in, 
                      *lasttemp =  (y+rowNum)*data_size_X+lastColumn+in;
    ...
    #pragma omp parallel shared(kernel, out, in)
    {
        ...
        for (int x = 0;x<data_size_X;x++){
            ...
            float first = 0, last = 0;
            for(int j = -1; j <= 1; j++){
                float * firstLineTemp = j*data_size_X+x + in ,
                      * secondLineTemp = (lastLineNum+j)*data_size_X+x+in;
                 int   kerPositionTemp =(1-j)*3;
                for(int i = -1; i <= 1; i++){
                    float* kernelPosition= (1-i)+kerPositionTemp+kernel;
                    if(x+i>-1 && x+i<data_size_X ){
                        if(j!=-1 && j!=data_size_Y) first += *kernelPosition * *(i + firstLineTemp);                    
                        if(lastLineNum+j!=-1 && lastLineNum+j!=data_size_Y) last  += *kernelPosition * *(i + secondLineTemp);
                    }
                }
            }
            out[x] = first;
